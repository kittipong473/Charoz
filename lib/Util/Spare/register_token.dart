...
class RegisterToken extends StatelessWidget {
  ...
  Future requestOTP(BuildContext context) async {
    VariableGeneral.auth.verifyPhoneNumber(
      phoneNumber: "+66" + phoneController.text,
      verificationCompleted: (PhoneAuthCredential credential) async {
        await VariableGeneral.auth.signInWithCredential(credential).then((value) {
          MyFunction().toast('ยืนยัน OTP สำเร็จ');
        });
      },
      verificationFailed: (FirebaseAuthException e) {
        EasyLoading.dismiss();
        MyDialog(context).singleDialog(e.code);
      },
  ...
  Future authenTokenFirebase(BuildContext context) async {
    ...
    await VariableGeneral.auth.signInWithCredential(credential).then((value) async {
